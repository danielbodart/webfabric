...
object Request{
  ...
  def post(path: String, form: FormParameters): Request = Request(HttpMethod.POST, path, HeaderParameters("Content-Type" -> "application/x-www-form-urlencoded"), QueryParameters(), form, emptyInput)
  ...
  def apply(request: HttpServletRequest):Request = {
    Request(request.getMethod, request.getPathInfo, HeaderParameters(request), QueryParameters(request), FormParameters(request), request.getInputStream)
  }
}